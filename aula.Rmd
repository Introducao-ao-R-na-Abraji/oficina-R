...
Já a função `rm_accent()` do pacote `abjutils` elimina o acento do texto.
...
Conforme já dito logo no início, os dados que estamso trabalhando estão disponíveis na internet. Vamos ver como podemos usar o R para baixar esses dados para seu ambiente computacional. Também vamos ver como salvar o rabalho que você fez em arquivo csv para ser aproveitado em outros usos que não seja a partir da linguagem R e por fim você vai prender a ler vários arquivos CSVs.
...
## 5.3) Leitura de vários CSVs

Os arquivos CSVs de 2020 estão em pasta zipada:
https://drive.google.com/file/d/1ix-VpE0aGmEOEZwN07VaXKs24AN5FMsn/view?usp=sharing
