...
Abaixo listamos os arquivos RDS que estão na nossa pasta e chamamos de "arquivos".

```{r}
arquivos <- list.files("data/pensionistas_2020")
```

Agora vamos informar dentro de `map_df()` o caminho ("data/pensionistas_2020/"), seguido pelo nome dos arquivos (estão guardados pelo dentro "arquivos"), a função `readRDS` (que faz a leitura de RDS) e também `.id = "source"` para uma coluna difereciar o arquivo de origem. 
